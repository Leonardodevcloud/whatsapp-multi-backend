...
// CORRIGIDO: salvar media_tipo no INSERT e UPDATE
...
async function criarRespostaRapida({ atalho, titulo, corpo, mediaUrl, mediaTipo, filaId, usuarioId }) {
  ...
  const resultado = await query(
    `INSERT INTO respostas_rapidas (atalho, titulo, corpo, media_url, media_tipo, fila_id, criado_por)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING *`,
    [atalhoLimpo, titulo.trim(), corpo.trim(), mediaUrl || null, mediaTipo || null, filaId || null, usuarioId]
  );
...
async function atualizarRespostaRapida({ id, dados }) {
  ...
  const camposPermitidos = ['titulo', 'corpo', 'media_url', 'media_tipo', 'fila_id'];
  const updates = [];
  const params = [];
  let idx = 1;
